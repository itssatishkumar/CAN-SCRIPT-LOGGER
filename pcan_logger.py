...
class PCANViewClone(QMainWindow):
    ...
    def start_logging(self, filename):
        try:
            ...
            # create CSV logger and attach to reader if connected (reader emits message_received)
            try:
                # CSV file saved next to TRC file
                out_dir = os.path.dirname(filename) or os.getcwd()
                self.csv_logger = CSVLogger(dbc_filename=os.path.join(os.path.dirname(__file__), "Marvel_3W_all_variant.dbc"),
                                            output_dir=out_dir,
