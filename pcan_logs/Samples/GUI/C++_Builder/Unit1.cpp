...
void __fastcall TForm1::btnParameterGetClick(TObject *Sender)
{
	...
	case 23:
		...
		if (stsResult == PCAN_ERROR_OK)
		{
			switch(iBuffer)
			{
				...
				case LAN_DIRECTION_READ_WRITE:
					...
					break;
				default:
					info = Format("The communication flow is: undefined (0x%.4X)", ARRAYOFCONST(((iBuffer))));
					break;
			}
			IncludeTextMessage(info);
		}
        ...
        //
    case 24:
        stsResult = m_objPCANBasic->GetValue(m_PcanHandle, PCAN_DEVICE_GUID, strBuffer, 256);
        if (stsResult == PCAN_ERROR_OK)
        {
            info = Format("The GUID of the device is %s", ARRAYOFCONST(((AnsiString)strBuffer)));
            IncludeTextMessage(info);
        }
        break;
