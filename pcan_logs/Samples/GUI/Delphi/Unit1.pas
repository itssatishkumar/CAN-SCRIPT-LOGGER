...
begin
  ...
  case cbbParameter.ItemIndex of
    ...
    23:
      begin
        stsResult := TPCANBasic.GetValue(m_PcanHandle, PCAN_LAN_CHANNEL_DIRECTION, PLongWord(@iBuffer),
          SizeOf(iBuffer));
        if (stsResult = PCAN_ERROR_OK) then
        begin
          if (iBuffer = TPCANBasic.LAN_DIRECTION_READ_WRITE) then
            Info := 'The communication flow is: bidirectional'
          else
            if (iBuffer = TPCANBasic.LAN_DIRECTION_READ) then
              Info := 'The communication flow is: incoming only'
            else
              if (iBuffer = TPCANBasic.LAN_DIRECTION_WRITE) then
                Info := 'The communication flow is: outgoing only'
              else
    ...
    24:
      begin
        stsResult := TPCANBasic.GetValue(m_PcanHandle, PCAN_DEVICE_GUID, strBuffer, 256);
        if (stsResult = PCAN_ERROR_OK) then
        begin
